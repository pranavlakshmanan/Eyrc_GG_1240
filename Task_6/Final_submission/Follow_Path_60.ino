...
* It iterates over each sensor, powers it on, waits for a short delay, reads the sensor value, waits for another short delay,
  and then powers off the sensor. It was done so that there won't be any reflection from other IR sensors.
...
* It then waits for 5 seconds before ending the function.
